                            ...
                            if isinstance(msg, str) and ('403' in msg.lower() or 'forbidden' in msg.lower()):
                                if 'http error 403' in msg.lower() or 'failed to open segment' in msg.lower():
                                    ...
                                    if self.error_tracker['count'] >= self.error_tracker['max_errors']:
                                        logger.error(f"Too many HLS 403 errors ({self.error_tracker['count']}), aborting download early")
                            ...
                            try:
                                ...
                                if '403' in error_msg or 'forbidden' in error_msg:
                                    ...
                                    if error_count >= hls_403_errors['max_errors'] or hls_403_errors['count'] >= hls_403_errors['max_errors']:
                                        logger.error(f"Multiple 403 errors detected (stderr count: {hls_403_errors['count']}, error msg count: {error_count}), aborting early")
                                        raise yt_dlp.utils.DownloadError(f"HLS segment download failed: {max(hls_403_errors['count'], error_count)} 403 Forbidden errors detected")
