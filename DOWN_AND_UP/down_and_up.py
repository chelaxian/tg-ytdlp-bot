            ...
            try:
                ...
                if "Requested format is not available" in error_message:
                    if not did_format_fallback:
                        logger.warning(f"Format not available error, retrying with format='best': {error_message}")
                        did_format_fallback = True
                        # Retry with format='best' as fallback
                        fallback_opts = attempt_opts.copy()
                        fallback_opts['format'] = 'best'
                        # Remove any format-specific extractor args that might conflict
                        if 'extractor_args' in fallback_opts:
                            fallback_opts['extractor_args'] = {k: v for k, v in fallback_opts['extractor_args'].items() if k != 'youtube'}
                ...
                if final_name:
                    ...
                    try:
                        ...
                        try:
                            ...
                            video_uploader_for_check = get_metadata_text_for_keyword_check(info_dict) if info_dict else ""
                            ...
                            # Use metadata for NSFW check (title, description, tags, and all other metadata fields)
