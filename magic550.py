...
def send_videos(
    ...
    tags_text: str = '',
    info_dict: dict = None,  # <--- добавим info_dict для передачи метаданных
):
    import re
    import os
    user_id = message.chat.id
    ...
    try:
        # --- ГЛАВЫ: если это YouTube и есть chapters, используем их как описание ---
        description = full_video_title
        if info_dict and ("youtube.com" in video_url or "youtu.be" in video_url):
        ...
        title_html, pre_block, blockquote_content, tags_block, link_block, was_truncated = truncate_caption(
            ...
            description=description,  # Chapters или обычное описание
            url=video_url,
...
# --- New function to format chapters ---
def format_chapters(chapters):
    ...
    lines = []
    for ch in chapters:
        ts = format_time(ch.get('start_time', 0))
        title = ch.get('title', '')
