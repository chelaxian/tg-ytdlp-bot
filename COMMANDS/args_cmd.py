...
def log_ytdlp_options(user_id: int, ytdlp_opts: dict, operation: str = "download"):
    ...
    When Config.VERBOSE_LOGGING is True, logs detailed ytdl_opts including
    format string, cookie file presence, proxy host (without credentials),
    and other yt-dlp options.
    
    Sensitive data is always redacted:
    - cookiefile path → '[SET]' or '[NOT_SET]'
