...
def askq_callback(app, callback_query):
    ...
    if is_playlist_with_range(original_text):
        ...
        # Проверяем кэш только для выбранного качества
        cached_videos = get_cached_playlist_videos(get_clean_playlist_url(url), data, requested_indices)
        
        if cached_videos:
            # Если есть кэш для выбранного качества - пересылаем
            ...
            # Если кэша для выбранного качества нет - скачиваем заново
            logger.info(f"askq_callback: no cache found for quality_key={data}, starting new download")
            if data == "mp3":
