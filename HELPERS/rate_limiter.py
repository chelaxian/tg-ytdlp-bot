...
# Throttle repeated save-failure logs (issue #460): a full disk (Errno 122)
# makes every 30s save cycle fail; logging each one produced 90 identical
# errors per 24h. Log at most once per 10 minutes.
    ...
    while not _save_stop.is_set():
        ...
        try:
            ...
            if now - _last_save_error_log_ts >= _SAVE_ERROR_LOG_INTERVAL:
                ...
                logger.error(
                    f"Rate-limiter save failed (logging throttled to 1/10min; "
