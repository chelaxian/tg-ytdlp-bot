...
def add_proxy_to_ytdl_opts(ytdl_opts: dict, url: str, user_id: int = None) -> dict:
    ...
    if proxy_config:
        ...
        if proxy_url:
            ...
            logger.info(f"Added domain-specific proxy for {url}: [redacted]")
        else:
            ...
            if proxies:
                ...
                for i, proxy_info in enumerate(proxies):
                    try:
                        ...
                        logger.info(f"Trying {url} with proxy from file {i+1}/{len(proxies)} ({proxy_info['type']}): [redacted]")
                        result = operation_func(current_opts, *args, **kwargs)
                        
                        if result is not None:
                            logger.info(f"Success with proxy from file {i+1}/{len(proxies)} ({proxy_info['type']}): [redacted]")
                            _proxy_failed_for_task.pop(task_key, None)
                            return result
                        else:
                            logger.warning(f"Operation returned None with proxy from file {i+1}/{len(proxies)} ({proxy_info['type']}): [redacted]")
    ...
    if user_id:
        try:
            ...
            if country_proxy_url:
                ...
                logger.info(f"Using proxy from file for country {selected_country} (gallery-dl)")
                return config
        except Exception as e:
            logger.warning(f"Error checking country proxy for user {user_id}: {e}")
            pass
    
    # Priority 2: User has proxy enabled but no country (AUTO) → do NOT add proxy to initial request
    if user_id:
        try:
            from COMMANDS.proxy_cmd import is_proxy_enabled
            proxy_enabled = is_proxy_enabled(user_id)
            if proxy_enabled:
                logger.info(f"User {user_id} in AUTO mode (no country) - not adding proxy to gallery-dl initial request")
                return config
        except Exception as e:
            logger.warning(f"Error checking proxy for user {user_id}: {e}")
            pass
    
    # Priority 3: Domain requires specific proxy
    logger.info(f"Checking domain-specific proxy for {url}")
    proxy_config = select_proxy_for_domain(url)
    if proxy_config:
        proxy_url = build_proxy_url(proxy_config)
