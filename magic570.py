    ...
    try:
        ...
        def try_download_audio(url, current_index):
            ...
            ytdl_opts = {
               ...
               'prefer_ffmpeg': True,
               'extractaudio': True,
               'playlist_items': str(current_index),  # Исправлено: теперь передаём реальный индекс
...
"""
            ...
            if playlist_name and playlist_name.strip():
                ...
                rename_name = sanitize_filename(f"{playlist_name.strip()} - Part {current_index}")
            else:
...
app.run()# Version 3.0.2 # embedded subtitles
import glob
import hashlib
import io
import logging
